...
class os_t:
	...
	def sched (self, task):
		...
		self.cpu.set_virtual_memory_registers(task.paddr_offset, task.paddr_max)
	...
	def allocate_contiguos_physical_memory_to_task (self, words, task):
		...
		for block in self.memory_blocks:
			if block["free"] and block["size"] >= words:
				# Encontrou um bloco livre com tamanho suficiente
				start_address = block["start"]
				end_address = block["start"] + words - 1
				...
				return task.paddr_offset, task.paddr_max
...
def syscall(self):
    ...
    if service == 0:
        ...
        # Serviço de impressão de string
        addr = self.cpu.get_reg(1)  # Endereço virtual da string
        if self.check_valid_vaddr(task, addr):
            # Converter endereço virtual em endereço físico
            paddr = self.virtual_to_physical_addr(task, addr)
            # Ler a string da memória
            string = self.memory.read_string(paddr)
            # Imprimir a string na console
            self.terminal.console_print(string)
        else:
            self.handle_gpf("invalid virtual address")
        ...
        num = self.cpu.get_reg(1)  # Número inteiro
        self.terminal.console_print(str(num))
